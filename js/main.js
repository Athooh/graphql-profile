document.addEventListener("DOMContentLoaded", () => {
  ...
  function displayUserInfo(userData) {
    ...
    const allxp = userData.transaction.filter((t) => t.type === "xp");
    const totalXPInBytes = allxp.reduce(
      (sum, t) => sum + t.amount,
      0
    );
    const receivedxp = userData.transaction.filter((t) => t.type === "up");
    const totalXPreceived = receivedxp.reduce(
      (sum, t) => sum + t.amount,
      0
    );
    const givenxp = userData.transaction.filter((t) => t.type === "down");
    const totalXgiven = givenxp.reduce(
      (sum, t) => sum + t.amount,
      0
    );
  ...
  function displayXPProgress(xpData) {
    ...
    const allxp = xpData.transaction.filter((t) => t.type === "xp");
    console.log(allxp);
    const totalXPInBytes = allxp.reduce(
      (sum, t) => sum + t.amount,
      0
    ...
    const totalXPInMB = (totalXPInBytes / 1000 / 1000).toFixed(2);
  ...
  function displayAudits(audits) {
    ...
    const sortedAudits = passedAudits.sort(
  ...
  function displaySkills(userData) {
    ...
    if (!skills || skills.length === 0) {
      skillsSection.innerHTML =
        "<h3>Skills</h3><p>No skills data available yet</p>";
      return;
    }
  ...
  function createGraphs(xpData) {
    ...
    graphql
      ...
      .then((progressData) => {
        const stats = progressData.progress.reduce(
          (acc, curr) => {
